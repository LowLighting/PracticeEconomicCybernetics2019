...
public class Cissoid implements Shape, Serializable, Cloneable, Transferable{
    ...
    Cissoid(int centerX, int centerY, int leftX, int leftY, int width, int height, int a) {
        super();
...
public Rectangle getBounds()
{
    ...
    class ShapeIterator implements PathIterator {
        ...
        public int currentSegment(float[] coordinate) {
            if (start) {
                coordinate[0] = (float) (2 * a * Math.pow(Math.tan(alpha), 2) / (1 + Math.pow(Math.tan(alpha), 2))) + centerX;
                coordinate[1] = (float) (2 * a * Math.pow(Math.tan(alpha), 3) / Math.pow(Math.tan(alpha), 2)) + centerY;
        ...
        public int currentSegment(double[] coordinate) {
            if (start) {
                coordinate[0] = (2 * a * Math.pow(Math.tan(alpha), 2) / (1 + Math.pow(Math.tan(alpha), 2))) + centerX;
                coordinate[1] = (2 * a * Math.pow(Math.tan(alpha), 3) / Math.pow(Math.tan(alpha), 2)) + centerY;
                start = false;
    ...
    static DataFlavor decDataFlavor = new DataFlavor(Cissoid.class, "Cissoid");
    ...
    static Cissoid getFromString(String s)
    {
        String[] arr = s.split("");
